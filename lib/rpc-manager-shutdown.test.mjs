...
test("an exact system prompt is projected onto requests even after SDK preflight resets it", async (t) => {
  const inner = makePromptInner((_message, options) => {
  ...
  // pi >= 0.86 derives request prompts from transcript system messages and makes
  // state.systemPrompt read-only, so the exact prompt is projected onto the
  // outgoing request instead: the head system message collapses to it, whatever
  // the SDK last wrote into state.
  const projected = await inner.agent.transformContext([
    { role: "system", content: "SDK base prompt", timestamp: 1 },
    { role: "user", content: "hello", timestamp: 2 },
  ]);
  assert.deepEqual(projected.map((message) => message.role), ["system", "user"]);
  assert.equal(projected[0].content, "context prompt");
  assert.equal(projected[1].content, "hello");

  await wrapper.send({ type: "prompt", message: "hello" });
  await nextTurn();

  // get_state keeps reporting the exact prompt (the UI's tools-disabled gate).
