...
// For #48 classification tests: anthropic is authenticated, the acme gateways
// are not, so `getAvailable()` drops their models while the full catalog
// (getModels) still lists them.
const AUTHENTICATED = new Set(["anthropic"]);

const partiallyAuthenticatedRuntime = {
  getAvailable: async () => MODELS.filter((m) => AUTHENTICATED.has(m.provider)),
  getModels: () => MODELS,
  hasConfiguredAuth: (providerId) => AUTHENTICATED.has(providerId),
};
...
test("reports patterns that match nothing but keeps the models that matched", async () => {
  ...
  assert.equal(result.warnings[0].code, "no-match");
  assert.equal(result.warnings[0].pattern, "ghost-gateway/*");
  assert.match(result.warnings[0].message, /ghost-gateway\/\*/);
});

test("rewords patterns for unauthenticated providers as credential warnings (#48)", async () => {
  const result = await resolveVisibleModels(partiallyAuthenticatedRuntime, ["acme-gateway/claude-opus-4-8"]);

  assert.deepEqual(refs(result), ["anthropic/claude-opus-5", "anthropic/claude-sonnet-5", "anthropic/claude-sonnet-4-6"]);
  assert.equal(result.warnings.length, 1);
  assert.equal(result.warnings[0].code, "unauthenticated-provider");
  assert.equal(result.warnings[0].pattern, "acme-gateway/claude-opus-4-8");
  assert.deepEqual(result.warnings[0].unauthenticatedProviders, ["acme-gateway"]);
  assert.match(result.warnings[0].message, /No models match pattern/);
});

test("classifies provider globs and strips thinking-level suffixes when attributing (#48)", async () => {
  const globbed = await resolveVisibleModels(partiallyAuthenticatedRuntime, ["acme-*:high"]);
  assert.equal(globbed.warnings.length, 1);
  assert.equal(globbed.warnings[0].code, "unauthenticated-provider");
  assert.deepEqual([...globbed.warnings[0].unauthenticatedProviders].sort(), ["acme-gateway", "acme-gateway-openai"]);
  assert.equal(globbed.warnings[0].pattern, "acme-*:high");
});

test("keeps the typo message when the pattern's providers are authenticated (#48)", async () => {
  const result = await resolveVisibleModels(partiallyAuthenticatedRuntime, ["anthropic/claude-haiku-9"]);

  assert.equal(result.warnings.length, 1);
  assert.equal(result.warnings[0].code, "no-match");
  assert.equal(result.warnings[0].pattern, "anthropic/claude-haiku-9");
  assert.equal(result.warnings[0].unauthenticatedProviders, undefined);
});

test("attributes bare patterns to the unauthenticated providers owning the model (#48)", async () => {
  const result = await resolveVisibleModels(partiallyAuthenticatedRuntime, ["gpt-5.6-sol"]);

  assert.equal(result.warnings.length, 1);
  assert.equal(result.warnings[0].code, "unauthenticated-provider");
  assert.deepEqual(result.warnings[0].unauthenticatedProviders, ["acme-gateway-openai"]);
});
