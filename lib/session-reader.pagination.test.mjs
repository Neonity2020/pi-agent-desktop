...
test("pagination loses no history when compaction kept entries fall outside the page", () => {
  ...
  for (const tail of [1, 3, 5, 50]) {
    ...
    while (page.hasMore) {
      ...
      // A page may run past `tail` only to reach the start of the turn it cut into.
      assert.ok(page.entryIds.length <= tail || page.messages[0]?.role === "user" || page.messages[0]?.customType === "compaction");
...
test("sliceActiveBranch: tail counts visible messages — toolResults ride along free", () => {
  ...
  // 10 visible cut mid-turn, so the window extends back to that turn's user question.
  ...
  assert.equal(users.length, 4); // 10 visible = 3⅓ turns → extended to 4 whole turns
...
test("sliceActiveBranch: a window that cuts into a long turn reaches back to its user message", () => {
  // One user question followed by 120 assistant/toolResult pairs — the tail
  // alone would open mid-turn and render the process flat until the next page.
  const entries = [];
  const push = (message) => entries.push({
    id: `e${entries.length}`,
    parentId: entries.length === 0 ? null : `e${entries.length - 1}`,
    type: "message",
    timestamp: new Date(1000 + entries.length).toISOString(),
    message,
  });
  push({ role: "user", content: "earlier" });
  push({ role: "assistant", content: "earlier answer" });
  push({ role: "user", content: "long task" });
  for (let i = 0; i < 120; i++) {
    push({ role: "assistant", content: [{ type: "toolCall", id: `tc${i}`, name: "bash", arguments: {} }] });
    push({ role: "toolResult", toolCallId: `tc${i}`, content: [] });
  }
  push({ role: "assistant", content: "done" });
  const ctx = buildSessionContext(entries, entries[entries.length - 1].id, { tail: 50 });
  assert.equal(ctx.entryIds[0], "e2");
