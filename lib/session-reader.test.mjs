...
test("session listing reads subagent relations and terminal status without reopening full session files", async (t) => {
  // The fork's incremental scanner reads the real sessions directory, so the
  // fixture must live inside an isolated agent dir (not a listAll mock).
  ...
  const filePath = join(dir, "sessions", "proj", "child.jsonl");
  const childId = "bounded-relation-child";
  const parentPath = join(dir, "sessions", "proj", "parent.jsonl");
...
test("hides transcript system messages and metadata entries while keeping entry ids aligned", () => {
  ...
  // tail=2 stops on a1, mid-turn, so the page reaches back to that turn's u1.
